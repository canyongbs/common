...
if (! $value) {
    $value = $this->computeStats();
    Cache::put('stats', $value, 60);
}
```
...
```php
$value = Cache::remember('stats', 60, fn () => $this->computeStats());
```
...
Without tags, invalidating a group of entries requires tracking every key. Tags let you flush atomically.
