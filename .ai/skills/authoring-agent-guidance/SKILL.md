...
- **Enforcement stops at common's own skills.** A spec-compliance Pest guard (`tests/SkillSpecComplianceTest.php`, with the house-convention checks in `tests/SkillConventionsTest.php`) validates every skill under `.ai/skills/`, so an over-length or malformed common skill fails CI. **App-authored skills are NOT covered by that guard** (in a consuming app they live under `.ai/overrides/skills/**`) — the app is responsible for keeping its own local skills within the spec limits, or they silently disappear with no test to catch it.
