...
- **Treat app mechanics as context, not literal steps.** The shared skills describe app-only concerns — multi-tenancy, Filament panels, Pennant feature flags (activated per tenant in tenanted apps, or globally/for the landlord in apps without tenants), zero-downtime migrations. Apply their *principles* when they fit the code you are changing, but do not expect that machinery to exist here. In particular, the `local-common-development` skill is an app-side workflow for linking this package into an app; it does not apply when you are already inside the package.
