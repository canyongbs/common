...
final class BrowserNotificationReportHandler implements ReportHandlerInterface
{
    ...
    public function handleReport(
        ...
        $messageData = $message->toArray()['data'] ?? null;

        if (! is_array($messageData) || ($messageData['common_browser_notification'] ?? false) !== true) {
            return;
        }
        ...
        if ($status === null || in_array($status, [408, 425, 429], true) || ($status >= 500 && $status < 600)) {
            throw RetryableBrowserNotificationDeliveryException::forStatus($status);
        }
    }
}
