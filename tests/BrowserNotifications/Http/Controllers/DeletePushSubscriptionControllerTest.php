...
it('deletes only the current users browser subscription', function () {
    ...
    $user->updatePushSubscription($endpoint, 'public-key', 'auth-token', ContentEncoding::aes128gcm);

    expect(PushSubscription::query()->count())->toBe(1);

    actingAs($user);

    deleteJson(route('common.browser-notifications.subscriptions.destroy'), [
        'endpoint' => $endpoint,
    ])->assertSuccessful();
